...
FTCPClinetRunnable::FTCPClinetRunnable(UMatchmakingSubsystem* InOwner)
	: Thread(nullptr)
	, Socket(nullptr)
	, bRun(true)
	, bConnected(false)
...
{
	Thread = FRunnableThread::Create(this, TEXT("FTCPClinetRunnable"), 0, TPri_Normal);
}
FTCPClinetRunnable::~FTCPClinetRunnable() {
	Stop();
	if (Thread) {
		Thread->Kill(true);
		delete Thread;
...
void FTCPClinetRunnable::Stop() {
	bRun = false;
}

void FTCPClinetRunnable::SendRawString(const FString& Message) {
	if (!Socket) return;
...
void FTCPClinetRunnable::HostNewGame(const FString& Name, int32 Port) {
	if (!Socket) return;
	// Obtain local ip
	bool bCanBind = false;
...
void FTCPClinetRunnable::RequestSessionList() {
